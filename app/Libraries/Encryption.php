...
class Encryption {
	...
	public function __construct(array $params = array())
	{
		...
		{
			dd('Encryption: Unable to find an available encryption driver.');
		}

		isset(self::$func_override) OR self::$func_override = (extension_loaded('mbstring') && ini_get('mbstring.func_override'));
		$this->initialize($params);

		if ( ! isset($this->_key) && self::strlen($key = env('CI_ENCRYPTION_KEY')) > 0)
		{
			$this->_key = $key;
		}
	...
	public function initialize(array $params)
	{
		if ( ! empty($params['driver']))
		{
			if (isset($this->_drivers[$params['driver']]))
			{
				...
				{
					dd('error', "Encryption: Driver '".$params['driver']."' is not available.");
				}
			}
			else
			{
				dd('error', "Encryption: Unknown driver '".$params['driver']."' cannot be configured.");
			}
		}
...
//			dd('debug', "Encryption: Auto-configured driver '".$this->_driver."'.");
		}
	...
	protected function _mcrypt_initialize($params)
	{
		if ( ! empty($params['cipher']))
		{
			...
			{
				dd('error', 'Encryption: MCrypt cipher '.strtoupper($params['cipher']).' is not available.');
			}
			else
			{
		...
		if ( ! empty($params['mode']))
		{
			...
			{
				dd('error', 'Encryption: MCrypt mode '.strtoupper($params['mode']).' is not available.');
			}
			else
			{
		...
		if (isset($this->_cipher, $this->_mode))
		{
			...
			{
				dd('info', 'Encryption: MCrypt cipher '.strtoupper($this->_cipher).' initialized in '.strtoupper($this->_mode).' mode.');
			}
			else
			{
				dd('error', 'Encryption: Unable to initialize MCrypt with cipher '.strtoupper($this->_cipher).' in '.strtoupper($this->_mode).' mode.');
			}
		}
	}
	...
	protected function _openssl_initialize($params)
	{
		...
		if ( ! empty($params['mode']))
		{
			...
			{
				dd('error', 'Encryption: OpenSSL mode '.strtoupper($params['mode']).' is not available.');
			}
			else
			{
		...
		if (isset($this->_cipher, $this->_mode))
		{
			...
			{
				$this->_handle = NULL;
				dd('error', 'Encryption: Unable to initialize OpenSSL with method '.strtoupper($handle).'.');
			}
			else
			{
				$this->_handle = $handle;
//				dd('info', 'Encryption: OpenSSL initialized with method '.strtoupper($handle).'.');
			}
		}
	}
